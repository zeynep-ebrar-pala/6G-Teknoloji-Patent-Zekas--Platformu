...
429 olursa <code>x-rate-limit-retry-after-seconds</code> kadar beklenir.
        ...
        "mno_live": {
            "bg_wait": "Ülke başına kilitli 3 MNO + Türk Telekom Springer metin sayımı arka planda ({done}/{total}). Menü kilitlenmez; sayı uydurulmaz.",
...
Bu sayfa <strong>Springer Nature Meta API</strong> kullanır. Bireysel e-posta yeter. Anahtar: <code>SPRINGER_API_KEY</code>.
            ...
            "mno_body": "Sorun: her Avrupa ülkesinde hangi işletmecinin 6G literatüründe daha görünür olduğu tek bakışta yok. Çözüm: ülke başına kilitli üç **MNO (Mobile Network Operator — mobil şebeke işletmecisi)** Springer Nature Meta metin aramasında sayılır; çubuk o üçün en yükseğidir (bağlılık facet’i değil). **Türk Telekom** aynı eksene Türkiye sorgusuyla konur; sıra, kaç ülke liderinin TT’den çok kaydı olduğuna göre okunur.",
            "mno_body_expert": "Sorgu biçimi: 6G + konu + tırnaklı firma adı + ülke İngilizce adı, Springer onlinedatefrom/to ile. Bu affiliation (bağlılık) facet değildir; Orange + France gibi metin eşleşmesi gürültü üretebilir. Sayı yoksa çubuk yok (0 uydurulmaz). Avrupa lider kümesi TR’sizdir. TT sırası = 1 + (lead_n > tt_n olan ülke lideri sayısı).",
            ...
            "mno_chart_title": "Ülke lideri MNO + Türk Telekom",
            "mno_chart_x": "Springer kayıt (metin araması)",
            "mno_col_country": "Ülke",
            "mno_col_lead": "Lider MNO",
            "mno_col_n": "Kayıt",
            "mno_col_three": "Kilitli 3 MNO",
            "mno_table_caption": "Sıralama ülke lideri (ve TT) kayıt sayısına göredir. Üç firma sütunu aynı sorgunun ham ölçümüdür; toplanmaz.",
            "mno_tt_row": "Türk Telekom — Türkiye metin araması (Avrupa kilitli 3’lüsünde değil)",
            "metric_eu_tt": "Türk Telekom kayıt",
            "metric_eu_tt_rank": "TT sırası (ülke liderleri + TT)",
...
On 429 the client waits <code>x-rate-limit-retry-after-seconds</code>.
        ...
        "mno_live": {
            "bg_wait": "Locked 3 MNOs per country plus Türk Telekom — Springer text counts in the background ({done}/{total}). The sidebar stays free; counts are not invented.",
...
This page uses the <strong>Springer Nature Meta API</strong>. A personal email is enough. Key: <code>SPRINGER_API_KEY</code>.
            ...
            "mno_body": "Problem: you cannot see, in one view, which operator is more visible in the 6G literature in each European country. Method: the locked three **MNOs (mobile network operators)** per country are counted in a Springer Nature Meta text search (not an affiliation facet); the bar is the highest of those three. **Türk Telekom** is placed on the same axis with a Türkiye query; rank is how many country-leaders have more records than TT.",
            "mno_body_expert": "Query shape: 6G + topic + quoted firm name + English country name, with Springer onlinedatefrom/to. This is not an affiliation facet; Orange + France style text matches can be noisy. No count → no bar (zeros are not invented). The European leader set excludes TR. TT rank = 1 + (country-leaders with lead_n > tt_n).",
            ...
            "mno_chart_title": "Country-leader MNO + Türk Telekom",
            "mno_chart_x": "Springer records (text search)",
            "mno_col_country": "Country",
            "mno_col_lead": "Leader MNO",
            "mno_col_n": "Records",
            "mno_col_three": "Locked 3 MNOs",
            "mno_table_caption": "Order follows country-leader (and TT) record counts. The three-firm column is the same query’s raw measurements; they are not summed.",
            "mno_tt_row": "Türk Telekom — Türkiye text search (not in a European locked trio)",
            "metric_eu_tt": "Türk Telekom records",
            "metric_eu_tt_rank": "TT rank (country-leaders + TT)",
            "metric_eu_tt_rank_n": "{rank} / {field}",
