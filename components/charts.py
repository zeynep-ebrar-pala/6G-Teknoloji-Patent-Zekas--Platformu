...
def render_technology_performance_bar(
    ...
    """Teknoloji KPI çubuğu — hız, gecikme, menzil, kazanç (makale sayısı değil)."""
    lang = get_lang()
    labels: List[str] = []
    vals: List[int] = []
    displays: List[str] = []
    ...
    y_title = f"{unit}" if unit else t("charts.perf_value")
    _apply_layout(
        fig,
...
def render_technology_performance_line(
    ...
    """Menzil–hız gibi performans eğrileri (literatür/demo sınıfı)."""
    lang = get_lang()
    xs: List[float] = []
    ys: List[float] = []
    labels: List[str] = []
...
def render_technology_performance_charts(tech_id: str) -> tuple[List[go.Figure], str]:
    """Teknoloji başına performans KPI grafikleri; caption metni döner."""
