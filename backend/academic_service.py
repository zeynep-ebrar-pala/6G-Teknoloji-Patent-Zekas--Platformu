...
class AcademicService:
    ...
    def get_bundle(region: str = "both", topic: Optional[str] = None) -> Dict[str, Any]:
        return literature_bundle(region, _norm_topic(topic), key_fingerprint(), "sp5", _live_fp())

    @staticmethod
    def get_summary(topic: Optional[str] = None) -> Dict[str, Any]:
        bundle = literature_bundle("both", _norm_topic(topic), key_fingerprint(), "sp5", _live_fp())
        meta = snapshot_meta()
    ...
    def get_most_cited_papers(topic: Optional[str] = None) -> List[Dict[str, Any]]:
        return list(
            literature_bundle("both", _norm_topic(topic), key_fingerprint(), "sp5", _live_fp()).get("cited") or []
        )
